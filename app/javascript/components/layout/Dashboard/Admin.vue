  ...
  <div class="columns">
    <app-card shadowless="true" class="column" padding="15">
      <p class="title has-text-facebook is-4">Información de Facebook
        <a v-show="facebookData && facebookData.user" href="/admin/facebook/auth" class="is-size-7">Refrescar</a>
      </p>
      ...
      <div v-else class="has-text-grey has-text-centered">
        <a href="/admin/facebook/auth" class="button is-facebook is-big is-flex">
  ...
  methods: {
    handleCreatePost: function (page) {
      this.$swal({
        ...
        preConfirm: (value) => {
          this.$axios.post('/admin/facebook/post', {
            id: page.id,
    ...
    fetchFacebookData: function () {
      const that = this
      this.$axios.get('/admin/facebook/data')
        .then(({data}) => {
