        ...
        <div class="columns is-is-fullwidth">
          <div class="column is-4" v-for="(publication, key) in campaign.publications" :key="key">
            <app-card nested="true">
              ...
              <a :href="`/designer/publications/${publication.id}`" class="button is-link is-rounded">Ver más</a>
              </div>
            </app-card>
          </div>
        </div>
  ...
  export default {
    ...
    methods: {
      fetchCampaigns: function () {
        const that = this
        this.$axios.get(`/designer/publications.json`)
        .then(({data}) => {
          console.log(data)
