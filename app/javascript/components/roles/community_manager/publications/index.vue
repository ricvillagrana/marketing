  ...
  <div v-if="campaigns">
    ...
    <div v-for="(campaign, key) in campaigns" :key="`campaign-${key}`">
      <div class="my-30">
        ...
        <p class="title is-4"><a :href="`/community_manager/campaigns/${campaign.id}`">{{ campaign.name }} <span class="tag">{{ campaign.fetchPublications.length }} publicaciones</span></a></p>
      </div>
      <div class="columns is-is-fullwidth">
        <div class="column is-3" v-for="(publication, key) in campaign.publications" :key="key">
  ...
  export default {
    ...
    methods: {
      fetchPublications: function () {
        const that = this
        this.$axios.get(`/community_manager/publicaitions.json`)
        .then(({data}) => {
